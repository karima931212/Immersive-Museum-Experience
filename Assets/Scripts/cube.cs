...
{
	SerialPort stream = new SerialPort("/dev/cu.HC-06-DevB",9600);
	//SerialPort stream = new SerialPort("/dev/cu.usbmodem1441",9600);
	...
	void Update()
	{
		...
		{
			target.transform.Translate(new Vector3(speed * Time.deltaTime,0,0));
		}
...
//			float az = Int32.Parse(dataRaw[2]) * acc_normalizer_factor;
			...
			float gz = Int32.Parse(dataRaw[1]) * gyro_normalizer_factor;
...
//			
						...
						//curr_angle_y += gy;
						curr_angle_z += gz;
			curr_x = curr_angle_x * factor2.Remap(-180,180,-90,90);
			...
			if(enableRotation)target.transform.rotation = Quaternion.Euler(-curr_x, -curr_angle_z * factor, 0);
		}
	}
